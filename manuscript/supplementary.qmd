...
Crucially, the SPARK estimation architecture operates as a hybrid continuous-discrete jump system. Following the measurement update at each heartbeat, the biological repolarization of the sinoatrial node is geometrically enforced by instantaneously resetting the integrated phase states ($I_p, I_s$) and their corresponding dimensions within the posterior covariance matrix ($P_{k|k}$) to exactly zero. Conversely, the latent autonomic states ($x_p, x_s$) and their covariances are strictly preserved across the boundary. This asymmetric reset mechanism ensures that the continuous, trans-beat memory of the autonomic nervous system is maintained while preventing artificial, non-physiological phase accumulation across sequential intervals.
...
We then apply the exact same mathematical mechanism utilized in the classical time-rescaling theorem: the probability integral transform. However, because the generative noise of the SPARK diffusion model is standard normal rather than exponential, the transformation requires the standard normal CDF, $\Phi(\cdot)$:
...
Let the continuous-time autonomic state vector be $\mathbf{X}(t) = [x_p(t), x_s(t), I_p(t), I_s(t)]^\intercal$, where $x_p(t)$ and $x_s(t)$ represent the instantaneous parasympathetic and sympathetic Ornstein-Uhlenbeck processes, and $I_p(t)$ and $I_s(t)$ represent their exact running integrals. 
...
Because this system is strictly linear time-invariant between observations, the exact discrete-time transition over any arbitrary stochastic inter-beat interval $\Delta t_k$ is solved analytically via the matrix exponential:
...
This exact transition matrix $\mathbf{F}_k$ allows the Kalman filter to propagate the state vector continuously across non-uniform biological time. Upon the registration of a macroscopic interval (the phase-domain boundary crossing), the integral states $I_p$ and $I_s$ are projected into the observation equation, and subsequently structurally reset to zero. This mathematical reset perfectly mirrors the biophysical depolarization and refractory resetting of the sinoatrial node, preserving absolute continuous-time fidelity without accumulation of discrete-time discretization errors.

## Explicit derivation of the exact transition matrix
...
For the bottom-left off-diagonal element governing the cumulative integral state ($I_p$), the series expansion operates exclusively for $n \ge 1$ (as the $n=0$ identity matrix $\mathbf{I}$ has a zero in this position). By extracting a factor of $-1/\kappa_p$ from the terms, we reconstruct the exact exponential series:

$$
\sum_{n=1}^{\infty} \frac{\Delta t_k^n}{n!} (-\kappa_p)^{n-1} = -\frac{1}{\kappa_p} \sum_{n=1}^{\infty} \frac{(-\kappa_p \Delta t_k)^n}{n!} 
$$

Recognizing that the right-side summation is simply the complete Taylor series for $e^{-\kappa_p \Delta t_k}$ minus the $n=0$ term (which equals $1$), the expression resolves analytically to the exact running integral:

$$
-\frac{1}{\kappa_p} \left( e^{-\kappa_p \Delta t_k} - 1 \right) = \frac{1 - e^{-\kappa_p \Delta t_k}}{\kappa_p}
$$

Because the parasympathetic and sympathetic pathways are mathematically decoupled within the continuous system matrix (i.e., they do not cross-talk), the exact same algebraic reduction applies symmetrically to the sympathetic parameters. The instantaneous sympathetic state at position (2,2) evaluates independently to:
...
Finally, because the entire right half of the block matrix $\mathbf{A}^n$ is identically zero for all $n \ge 1$, the integral preservation states at positions (3,3) and (4,4) are populated solely by the $n=0$ identity matrix $\mathbf{I}$, thereby remaining exactly $1$. 

Assembling these independent, exact scalar solutions back into the full $4 \times 4$ geometry yields the final closed-form transition matrix $\mathbf{F}_k$. This derivation explicitly proves that the accumulation of both the parasympathetic and sympathetic autonomic integral states is evaluated natively across continuous time, entirely bypassing the discretization errors inherent to numerical integration schemes (e.g., Euler or Runge-Kutta).
